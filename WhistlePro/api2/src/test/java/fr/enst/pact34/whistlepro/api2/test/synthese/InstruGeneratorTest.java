...
public class InstruGeneratorTest {
    ...
    public void testGeneration()
    {
        ...
        Signal output = generator.generate(PisteMelodie.Instrument.Piano, 0.010,100);
        assertEquals(0.010*Fs,output.length(),Double.MIN_VALUE);

        output = generator.generate(PisteMelodie.Instrument.Piano, 0.020,100);
        assertEquals(0.020*Fs,output.length(),Double.MIN_VALUE);

        output = generator.generate(PisteMelodie.Instrument.Piano, 0.030,100);
